...
class TestDataFileRequestJacksonTest {
  @Autowired
  private JsonMapperService jsonService;

  @Test
  void shouldJacksonSerializeCorrectly() throws IOException {
    final TestDataFileRequest dto = TestDataFileRequestTest.newInstance();

    final String json = jsonService.toJson(dto);
    final TestDataFileRequest fromJson = jsonService.fromJson(json, TestDataFileRequest.class);
