...
class TestDetailResultJacksonTest {
  @Autowired
  private JsonMapperService jsonService;

  @Test
  void shouldJacksonSerializeCorrectly() throws IOException {
    final TestDetailResult dto = TestDetailResultTest.newInstance();

    final String json = jsonService.toJson(dto);
    final TestDetailResult fromJson = jsonService.fromJson(json, TestDetailResult.class);
