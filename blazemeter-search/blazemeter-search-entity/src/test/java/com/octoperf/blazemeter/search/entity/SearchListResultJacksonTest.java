...
class SearchListResultJacksonTest {
  @Autowired
  private JsonMapperService jsonService;

  @Test
  void shouldJacksonSerializeCorrectly() throws IOException {
    final SearchListResult<Integer> dto = SearchListResultTest.newInstance();

    final String json = jsonService.toJson(dto);
    final SearchListResult<Integer> fromJson = jsonService.fromJson(json, new TypeReference<>() {
    });
